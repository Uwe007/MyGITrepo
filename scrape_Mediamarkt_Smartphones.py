...
di=[]
while soup.text.find(vp,position)>=0: 
    di.append(soup.text[soup.text.find(quotes,soup.text.find(quotes,soup.text.find(i,soup.text.find(vp,position)))+1)+1 : soup.text.find(quotes,soup.text.find(quotes,soup.text.find(quotes,soup.text.find(i,soup.text.find(vp,position)))+1)+1)])
    position = soup.text.find(quotes,soup.text.find(quotes,soup.text.find(i,soup.text.find(vp,position)))+1)+1

# lookup string p="price"  
position=0 # startvalue
price=[]
while soup.text.find(vp,position)>=0: 
...
import pandas as pd
mediamarkt_smartphones= pd.DataFrame({"name":name,"id" :di,"price" :price,"brand" :brand,"ean" :ean,"dimension25":dimension25,"dimension26" :dimension26,"dimension24" :dimension24,"category" :category,"dimension09" :dimension09,"dimension10":dimension10})

#mediamarkt_smartphones
mediamarkt_smartphones
